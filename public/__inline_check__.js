    ...
    container.innerHTML = apps.map((app) => `
      <div class="app-card">
        ...
        <div style="margin-top:.8rem;display:flex;align-items:center;gap:.8rem;flex-wrap:wrap">
          ...
          ${app.status === 'Approved' ? '<button class="btn btn-sm btn-outline" onclick="document.querySelectorAll(
</body>
</html>
.ptab
</body>
</html>
)[3].click();renderDownloads()">📥 Download Pass</button>' : ''}
        </div>
      </div>
    `).join('');
  } catch {
...
async function handleAdminLogin() {
  ...
  try {
    ...
    const data = await safeJson(response);
    if (!response.ok) {
      showToast('❌ Login Failed', data.message || 'Invalid admin credentials.', 'error');
      return;
    }
...
function wireFileInputs() {
  ...
  photoInput.addEventListener('change', () => {
    document.getElementById('ap-photo-name').textContent = photoInput.files[0]?.name || 'No photo selected';
  });
  documentInput.addEventListener('change', () => {
    document.getElementById('ap-document-name').textContent = documentInput.files[0]?.name || 'No document selected';
  });

  document.getElementById('ap-type').addEventListener('change', () => {
    refreshValidityLabels();
    updatePreview();
  });
  document.getElementById('ap-validity').addEventListener('change', updatePreview);
}
